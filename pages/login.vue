<template>
  <div class="h-full max-w-[1400px] mx-auto sm:rounded-[30px] overflow-hidden flex relative">
      ...
      <transition name="fade">
        ...
        >
          ...
          >
            ...
            <button
              class="text-xl py-3 bg-[#84754e] rounded-[40px] transition-all hover:bg-[#a1936e]"
            >
              Sign in
            </button>
          </Form>
          ...
          <p class="mt-5 text-center text-sm text-slate-600">
            ...
            <span
              class="text-blue-600 cursor-pointer hover:opacity-70"
              @click="chagePageType('register')"
              >Register</span
            >
          </p>
        </div>
    ...
    <div class="bg-[#040404] w-0 lg:w-1/3"></div>
    <img
      class="absolute h-auto bottom-0 translate-y-[40%] right-0 translate-x-[40%] w-[1000px] scale-125 opacity-80 lg:scale-100 lg:opacity-100 lg:left-2/3 lg:-translate-x-1/2 lg:top-[50%] lg:-translate-y-1/2 lg:w-[500px] xl:w-[600px]"
      src="@/assets/coffee_bg.png"
    />
  </div>
</template>
...
const registerSchema = object({
  ...
  confirmPassword: string()
    .required()
    .min(8)
    .oneOf([YRef('password'), null], 'Passwords must match')
})
